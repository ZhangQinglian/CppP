...
JNIEXPORT void JNICALL Java_com_zqlite_cpp_cppp_MainActivity_seekIddTo(
    ...
    unsigned int transparentColor = gifParser->getGCT()->getColorIntAt(
            oneFrame->graphicControlExt->getTransparentInde());
    ...
    jclass clazz = env->GetObjectClass(obj);
    jmethodID drawOneFrameId = env->GetMethodID(clazz, "drawOneFrame", "(IIII[IIZJ)V");
...
extern "C"
JNIEXPORT void JNICALL Java_com_zqlite_cpp_cppp_MainActivity_delete(
        JNIEnv *env,
        jobject obj,
        jlong tobeDelete
) {
    OneFrame *ptr = (OneFrame *) tobeDelete;
    ptr->graphicControlExt->release();
    ptr->iddStruct->imageDescriptor->release();
    delete ptr->iddStruct->bitmapArray;
    delete ptr->iddStruct->imageDescriptor;
    delete ptr->graphicControlExt;
    delete ptr->iddStruct;
    delete ptr;
}